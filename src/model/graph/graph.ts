...
import { type Connection, type ConnectionJSON, ConnectionType, type Port } from '../connections';
...
export class Graph {
  ...
  protected convertersMap: Record<
    string,
    {
      from: ConverterFrom;
      to: ConverterTo;
  ...
  createFlowConnection(from: Node | string, to: Node | string, fromPort?: string, toPort?: string): Connection {
    ...
    const fromPortID = fromPort
      ? fromNode.getFlowOutput(fromPort)
  ...
  createDataConnection(
    from: { node: Node | string; port: string },
    to: { node: Node | string; port: string }
  ): Connection {
  ...
  addConnection(connection: Connection) {
    ...
    const existingPair = this.connections.find(
      (c) =>
