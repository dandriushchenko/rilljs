...
class DummyDatum extends Datum {
  ...
  get defn() {
    return DummyDatum.defn;
  }
  get ctor() {
    return DummyDatum as unknown as DatumConstructor;
  }
  toJSON() {
    return this.value;
  }
  static fromJSON(v: unknown) {
    return v;
  }
}
...
describe('Graph', () => {
  ...
  it('custom Add Connection invalid', () => {
    ...
    expect(() => {
      g.addConnection({
        ...
        source: { node: n1.nodeID, port: 'vOut' },
        destination: { node: 'Y', port: 'vIn' },
      });
    }).toThrow(ConnectionIsNotValid);
    expect(() => {
      g.addConnection({
        ...
        source: { node: n1.nodeID, port: 'fOut' },
        destination: { node: 'Y', port: 'fIn' },
      });
    }).toThrow(ConnectionIsNotValid);
  });
  ...
  it('JSON ser/de', () => {
    ...
    const registry = new Registry([TestNode], [DummyDatum]);
