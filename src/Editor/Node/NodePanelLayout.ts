...
export function calcNodeAndPortsLayout(
  node: Node,
  design: NodeDesign,
  theme: CanvasTheme,
  portsCentered: boolean
): NodeLayout {
  ...
  const res: NodeLayout = {
    ...
    valuesOut: {},
    height:
      theme.node.header.height + flowLines * theme.node.ports.flow.height + valuesLines * theme.node.ports.value.height,
    classCaption: {
      x: 0,
      y: -15,
    },
    ...
    res.valuesIn[v.id] = {
      port: {
        ...
        y:
          ...
          vi * theme.node.ports.value.height +
