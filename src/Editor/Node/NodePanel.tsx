...
const NodePanelImpl = (props: NodePanelProps) => {
  const { node, design, selected, readonly, invalid, className } = props;
  ...
  return (
    <g transform={translate} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={classes}>
      {node.nodeName && (
        <TextBox
      ...
      <Backdrop width={design.width} height={height} onMouseDown={onMouseDown} />
      <Header node={node} draggable={!readonly} width={design.width} design={design} />
      <Content width={design.width} height={height}>
        {flowLines > 0 && (
          <>
            {flowsIn.map((f) => (
              <FlowPort
      ...
      {invalid && (
        ...
        >
          {Icons[themeCanvas.node.error.icon].map((d, i) => (
            <path key={i} d={d} fillRule='evenodd' />
          ))}
        </g>
      )}
      {prefs.mode === DesignViewMode.Detailed && node.defn.description && (
        <TextBox
