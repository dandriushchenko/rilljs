...
import { type Node, type NodeDesign, type Coords } from '../../../model';
...
export function Header(props: NodeHeaderProps) {
  const { node, width, design, className, style, draggable } = props;
  ...
  }, [mouseDownPos, dragStartNodes, actions, options, theme]);
  ...
  function onMouseDown(event: React.MouseEvent) {
    ...
    const dragStartNodesList = actions
      .findNodes((n) => (dragAll && n.selected) ?? n.node.nodeID === node.nodeID)
      .map((n) => ({
        id: n.node.nodeID,
        pos: {
          x: n.design.x,
  ...
  return (
    <g onMouseDown={onMouseDown} className={className} style={style}>
      <rect
        x={1}
        y={1}
        width={width - 2}
