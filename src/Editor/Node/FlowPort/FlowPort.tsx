...
import { type IOFlow, type Rect, type Coords, type Port } from '../../../model';
...
export const FlowPort = React.memo((props: FlowPortProps) => {
  const { pos, textRect, textAlignment, flow, port, readonly } = props;
  ...
  return (
    <g className={theme.base}>
      <PortZone x={pos.x} y={pos.y + halfHeight} port={port} readonly={readonly} className={theme.flow.class} />
      {flow.name && (
        <TextBox
          pos={textBox}
          text={flow.name}
