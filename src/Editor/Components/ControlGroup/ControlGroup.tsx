...
export function ControlGroup(props: React.PropsWithChildren<ControlGroupProps>) {
  const { label, labelHelp, labelFor, inline, help, fill, disabled, className, style, children } = props;
  ...
  return (
    <div className={classes} style={style}>
      {label && (
        <label className={theme.label} htmlFor={labelFor}>
          {label} <span className={theme.textMuted}>{labelHelp}</span>
        </label>
      )}
      <div>
        {children}
        {help && <div className={theme.help}>{help}</div>}
      </div>
    </div>
  );
}
