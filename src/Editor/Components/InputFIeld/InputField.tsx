import React, { type CSSProperties, useState, useRef, useLayoutEffect, useContext, useMemo } from 'react';
import { type Theme, ThemeContext } from '../../theme';
import { type IntentProps, type BaseProps } from '../props';
...
export function InputField(props: InputFieldProps) {
  ...
  useLayoutEffect(() => {
    if (rightElRef.current && Math.abs(rightElRef.current.clientWidth - rightElementWidth) > 2) {
      setRightElementWidth(rightElRef.current.clientWidth);
    }

    if (leftElRef.current && Math.abs(leftElRef.current.clientWidth - leftElementWidth) > 2) {
  ...
  function renderLeftElement() {
    ...
    return <span ref={leftElRef}>{leftElement}</span>;
  }
  ...
  function renderRightElement() {
    ...
    return <span ref={rightElRef}>{rightElement}</span>;
  }
