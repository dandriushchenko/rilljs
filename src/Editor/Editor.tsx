...
import { ConnectionType, type Coords, type Design, distance, type Graph, type Node, type Rect } from '../model';

import { type DesignOptions, type Options, OptionsContext, useOptions } from './options';
import { defaultTheme, type Theme, ThemeContext } from './theme';
import {
  type ModelActions,
  ModelActionsContext,
...
import { EditorDialogsContext, type RillEditorRef } from './ref';
import { useResizeObservable } from './hooks';
import { defaultViewPrefs, type ViewPrefs, ViewPrefsContext } from './prefs';
...
const RillEditorImpl: ForwardRefRenderFunction<RillEditorRef, RillEditorProps> = (props, fref) => {
  const {
    graph,
    design,
    theme = defaultTheme,
  ...
  const editorDialogs = useMemo(() => getEditorDialogs(modelActions, options, dims), [modelActions, options, dims]);
  ...
  const editingConnection = modelView.editingConnection?.reference
    ? modelView.connections[modelView.editingConnection.reference]
    : undefined;
      ...
      <ConnectionLine
        ...
        fromCoordsOverride={
          modelView.editingConnection.anchorPort.port === editingConnection.connection.destination.port
            ? modelView.editingConnection.targetPos
            : undefined
        }
        toCoordsOverride={
  ...
  function renderHighlight() {
    ...
    const conns = Object.values(modelView.connections).filter(
      (c) =>
        c.connection.source.node === modelView.nodeHighlight ||
        c.connection.destination.node === modelView.nodeHighlight
    );

    return [...conns.map((c) => renderConnection(c, false)), renderNode(modelView.nodeHighlight, false)];
  }
              ...
              <ModelViewContext.Provider value={modelView}>
                <EditorDialogsContext.Provider value={editorDialogs}>
                  <div
                    ...
                    onKeyUp={onKeyUp}
                  >
                    <ControlsPanel controls={prefs.controls} editorElement={editorElement} readonly={readonly}>
                      {prefs.controlsChildren}
                    </ControlsPanel>
                    <svg className={theme.classes.svg} ref={svgRef}>
                      <defs>
                        <Styles />
                        <Icons />
                      </defs>
                    ...
                    />
                    <EditOverlay nodes={modelView.nodesSelected} allowMultiNodeEdit={prefs.allowMultiNodeEdit} />
                    {/* <EditorMenu
                    target={ref.current}
                /> */}
                  </div>
