...
import { rectsOverlap, type Coords } from '../model/types';
import {
  type Node,
  type Graph,
  type Design,
...
function buildDefaultState(graph: Graph, maybeDesign: Design | undefined, options: Options): ModelView {
...
function buildActionsState(
  ...
  function adjustForPanAndScale(coords: Coords): Coords {
    ...
    if (currentPan.x === 0 && currentPan.y === 0 && viewState.scale === 1.0) {
      return coords;
    }
  ...
  function doPan(action: PanView) {
    ...
    if (finalPan.x < bb.x) {
      finalPan.x = bb.x;
    } else if (finalPan.x > bb.x + bb.width) {
      finalPan.x = bb.x + bb.width;
    }

    if (finalPan.y < bb.y) {
      finalPan.y = bb.y;
    } else if (finalPan.y > bb.y + bb.height) {
  ...
  function doZoom(action: ZoomView) {
    ...
    } else if (finalZoom > options.design.maxScale) {
  ...
  function doReorderNode(action: ReorderNode) {
    ...
    switch (order) {
      ...
      case NodeOrder.SendToBack:
        newIndex = 0;
        break;
      case NodeOrder.SendToFront:
        newIndex = viewState.nodesOrder.length - 1;
        break;
    }

    newIndex--;
    const nodesOrder = [...viewState.nodesOrder];
    nodesOrder.splice(index, 1);
    if (newIndex < 0) {
      newIndex = 0;
    } else if (newIndex > nodesOrder.length) {
  ...
  function doDeleteNodes(action: DeleteNodes) {
    ...
    const linkedConnections = findConnections(
      (c) => action.ids.includes(c.connection.source.node) || action.ids.includes(c.connection.destination.node)
    );
    ...
    Object.values(viewState.nodes).forEach((n) => {
      revalidateNode(n.node.nodeID, false);
    });
    return {
      success: true,
      redraw: true,
  ...
  function doUpdateConnection(action: UpdateConnection) {
    const { source, destination, disabled, id } = action;
    ...
    Object.keys(checkNodes).forEach((cn) => {
      revalidateNode(cn, false);
    });
  ...
  function doCreateConnection(action: CreateConnection) {
    const { from, to, connectionType } = action;
  ...
  function compressor(action: ActionType): ActionCompressor<any> | undefined {
    switch (action.type) {
      case 'PanView':
        return PanViewCompressor;
      case 'ZoomView':
        return ZoomViewCompressor;
      case 'SelectNodes':
        return SelectNodesCompressor;
      case 'MoveNodes':
        return MoveNodesCompressor;
      default:
        return undefined;
    }
  }
  ...
  function doAction<D>(action: ActionType): D | undefined {
    ...
    switch (action.type) {
      case 'PanView':
        actionResult = doPan(action);
        break;
      case 'ZoomView':
        actionResult = doZoom(action);
        break;
      case 'ReorderNode':
        actionResult = doReorderNode(action);
        break;
      case 'CreateNodes':
        actionResult = doCreateNodes(action);
        break;
      case 'DeleteNodes':
        actionResult = doDeleteNodes(action);
        break;
      case 'SelectNodes':
        actionResult = doSelectNodes(action);
        break;
      case 'MoveNodes':
        actionResult = doMoveNodes(action);
        break;
      case 'CreateConnection':
        actionResult = doCreateConnection(action) as unknown as ActionResult<D>;
        break;
      case 'UpdateConnection':
        actionResult = doUpdateConnection(action);
        break;
      case 'DeleteConnections':
        actionResult = doDeleteConnections(action);
        break;
      default:
  ...
  function beginConnectionEdit(port: Port, pos: Coords) {
    ...
    const existingConnections = findConnections(
      (c) =>
  ...
  function finishConnectionEdit() {
    ...
    if (editConn.reference) {
      ...
      const disconnectedNode =
        existing.connection.source.node !== from.node && existing.connection.source.node !== to.node
          ? existing.connection.source.node
          : existing.connection.destination.node;
    ...
    if (hooks.onPortsConnected) {
      hooks.onPortsConnected(connectionID, from, to);
    }
  }
  ...
  function revalidateConnection(id: string, commitRedraw = true) {
    ...
    if (c.connection.type === ConnectionType.Flow) {
      invalid = isValidConnection(c.connection.source, c.connection.destination)
        ? undefined
  ...
  function isValidConnection(fromPort: Port, toPort: Port) {
    ...
    if (
      from.node.nodeID === to.node.nodeID &&
