import React, { type SVGTextElementAttributes, useMemo } from 'react';
...
export const TextBox = React.memo((props: TextBoxProps) => {
  ...
  return (
    <text
      style={style}
      dominantBaseline={lines.dominantBaseline as SVGTextElementAttributes<SVGTextElement>['dominantBaseline']}
      textAnchor={lines.textAnchor as SVGTextElementAttributes<SVGTextElement>['textAnchor']}
      className={className}
    >
      {lines.wordsLines.map(({ line }, index) => (
        <tspan key={index} x={lines.x + pos.x} y={lines.y + pos.y} dy={index * lines.lineHeight}>
          {line}
        </tspan>
      ))}
    </text>
