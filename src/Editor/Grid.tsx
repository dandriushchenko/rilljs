...
import { type Coords, rectFromPoints, type Rect, type Dimensions } from '../model';
import { ModelActionsContext, type ModelActions } from './model';
...
export const Grid = React.memo((props: React.PropsWithChildren<GridProps>) => {
  const { scale, coords, disabled, viewport } = props;
  ...
  }, [panStartOffset, mouseDownPos, actions, disabled, scale, state, coords]);
  ...
  return (
    <g transform={`scale(${String(scale)}, ${String(scale)}),translate(${String(coords.x)},${String(coords.y)})`}>
      <rect
        x={gridOffset.x}
        y={gridOffset.y}
      ...
      />
      {props.children}
      {lassoRect && <Lasso rect={lassoRect} />}
    </g>
  );
});
