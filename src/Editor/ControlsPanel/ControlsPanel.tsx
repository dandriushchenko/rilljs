...
import { copyModelSelectionJSON, pasteModelJSON, type ModelChunkJSON } from '../utils';
...
export function ControlsPanel(props: React.PropsWithChildren<ControlsPanelProps>) {
  const { children, editorElement, readonly, controls = {} } = props;
  ...
  const buttons = useMemo(() => {
    async function onPaste() {
      ...
      try {
        pasteModelJSON(JSON.parse(text) as Partial<ModelChunkJSON>, actions, registry);
      } catch (e) {
  ...
  function onCopySelected() {
    ...
    void copyToClipboard(JSON.stringify(json));
  }

  return (
    <div className={theme.controlsPanel}>
      <div
      ...
      >
        {buttons}
        {buttons.length > 0 && children && <Divider />}
        {children}
      </div>
      {actions.getSelectedNodes().length > 0 && (
        <div
          ...
          onMouseUp={stopPropagation}
        >
          {!readonly && <Button icon='trash' title='Delete selected' onClick={onDeleteSelected} />}
          <Button icon='duplicate' title='Copy selected' onClick={onCopySelected} />
        </div>
      )}
    </div>
  );
