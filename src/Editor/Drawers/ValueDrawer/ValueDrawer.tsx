...
function drawDatum<DT, T extends Datum<DT>>(
  ...
  if (!DataDrawer) {
    if (options.debug) {
      return (
        <span className={theme.classes.textDebug}>
...
export function ValueDrawer<DT, T extends Datum<DT>>(props: ValueDrawerProps<DT, T>) {
  ...
  const { label, labelHelp, help } = useControlGroupLabels(value);

  const changeHandler = handleDatumValueChange<DT, T>(value, actions, node, () => {
    redraw({});
  });

  return (
    <ControlGroup label={label} help={help} labelHelp={labelHelp}>
