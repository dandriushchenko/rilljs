...
import { type DialogProps, Dialog, type SelectItemRendererProps, Select, Switch } from '../Components';
...
export const SnippetsDialog = React.memo((props: SnippetsDialogProps) => {
  const { isOpen, onClose, usePortal, snippets = [] } = props;
  ...
  function snippetRenderer(snippetProps: SelectItemRendererProps<Snippet>) {
    ...
    return (
      ...
      >
        ...
        <small className={theme.textMuted}>{snippet.description}</small>
      </div>
    );
  }
    ...
    <Dialog
      ...
      title={
        ...
        >
          ...
          <Switch checked={replaceAll} onChange={toggleReplace} label='Replace Content with Snippet' />
        </div>
      }
    >
      <Select<Snippet>
