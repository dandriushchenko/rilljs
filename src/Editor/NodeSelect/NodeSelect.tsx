...
export function NodeSelect(props: NodeSelectProps) {
  const { onSelect, predicate } = props;
  ...
  function nodeRenderer(itemProps: SelectItemRendererProps<Node>) {
    ...
    return (
      ...
      >
        <span style={design?.color ? { color: design.color } : undefined}>{defn.name}</span>
        <small className={theme.textMuted}>{defn.description}</small>
      </div>
    );
  }
  ...
  return <Select<Node> items={nodes} itemRenderer={nodeRenderer} itemPredicate={nodePredicate} onSelect={onSelect} />;
}
